...
object SortDomain {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val autousers =
      raw_autousers.select((explode(col("autousers"))).alias("users"))
    val users =
      autousers.withColumn("id", autousers.col("users").cast(LongType))
    ...
    val sites =
      raw_sites.withColumn("user_id", raw_sites.col("tmp_id").cast(LongType))
    ...
    val tg = domains
      .join(users, domains.col("user_id") === users.col("id"), "left")
      .withColumn(
        "auto-flag",
        when(domains.col("user_id") === users.col("id"), 1).otherwise(0)
      )
    ...
    val rel = tg_res
      .groupBy(col("domain"), col("auto-flag"))
      .agg(count("domain"), col("auto-flag"))
      .where(col("auto-flag") === 1)
